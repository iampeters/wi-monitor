<?php
    ...
    if (isset($_SESSION['username']) && isset($_SESSION['subject_id'])) {
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            if (($logged_in_user_id != $tbl_user_id) && empty($opponent)) {
                ...
                if (!$query1) {
                    ...
                    if ( mysqli_num_rows( $select_tag ) > 0) {
                        ...
                        $data->opponent_name = $player_row['username'];
                        $data->player_name = $opponent_row['username'];
            ...
            } else if (($logged_in_user_id == $tbl_user_id ) && empty($opponent)) {
                echo '{
                    "message" : "You have a quiz to complete",
                    "username" : "'.$username.'",
                    "subject" : "'.$subject.'"
                }';
            } else if (($logged_in_user_id == $tbl_user_id || $logged_in_user_id == $opponent) && $has_ended == false ) {
                echo '{
                    "message" : "You have a quiz to complete",
                    "username" : "'.$username.'",
                    "subject" : "'.$subject.'"
